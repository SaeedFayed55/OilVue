        ...
        <template #header>
            ...
            <div class="box" style=" margin-right : -300px; margin-top:20px">
                ...
                <select style="border:1px solid gray; border-radius : 2px solid red ; " v-model="selectedData" @change="populateTable()">
                    ...
                    <option value="" selected disabled hidden><i class='bx bx-home'></i> من فضلك اختار نوع المركبه </option>

                    <option v-for="(i, key) in getvehiclesTypesData_2" :key="key" :value="i.VEHICLE_TYPE_ID">
                        {{ i.VEHICLE_TYPE_NAME }}
...
export default {
    ...
    created() {
        this.$store.dispatch("getvehiclesTypesData_2")
    },

    computed: {
        getvehiclesTypesData_2() {
            return this.$store.getters.vehiclesTypesGetter_2
        },
        getvehiclesData() {
