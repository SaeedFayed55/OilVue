...
<div style="display : flex ; justify-content : center">
    ...
    <vs-table>
        <template #header>
            ...
            <div class="box" style=" margin-right : -300px; margin-top:20px">
                ...
                <select style="border:1px solid gray; border-radius : 2px solid red ; " v-model="selectedData" @change="populateTable()">
                    ...
                    <option value="" selected disabled hidden><i class='bx bx-home'></i> من فضلك اختار العام البحثى </option>
            ...
            <vs-dialog v-model="createActive">
                ...
                <div class="dialog" style="display : block ; direction : rtl ; justify-content: center ; left : -20px">
                    <div style="display : flex">
                        <label> الفتره التدريبيه</label>
                        <vs-input style="left : -20px ; width : 70% ;direction : rtl" v-model.lazy="name" @keypress.enter="createData(name), createActive = false" />
                    </div>
                    <br>
        ...
        <template #tbody v-if="activeTable">
            <vs-tr :key="i" v-for="(tr, i) in $vs.getPage($vs.getSearch(getPeriod, search), page, max)" :data="tr">
                <vs-td style="width : 12% ; text-align : center ; margin-left:70px ">
                    <a @click="deleteActive = true, id = tr.PERIOD_ID">
                ...
                <vs-td style="width : 12% ; text-align : center ; margin-left:70px ">
                    <a edit @click="editActive=true, id=tr.PERIOD_ID, name=tr.PERIOD_NAME">
                    ...
                    <vs-dialog v-model="editActive">
                        <template #header>
                            <strong>تعديل الفترة التدريبية</strong>
                        </template>
                        <div class="dialog" style="display : block ; direction : rtl ; justify-content: center ; left : -20px">
                            <vs-input style="width:70% ; left : -50px" v-model.lazy="name" @keypress.enter="updateData(id, name), editActive = false" />
                            <br>
                            <vs-button success style="left : -120px ; width : 25%" class="btnSave" @click="updateData(id, name), editActive = false">حفظ</vs-button>
                        </div>
                    </vs-dialog>
                </vs-td>
        ...
        <template #footer>
            <vs-pagination v-model="page" :length="$vs.getLength($vs.getSearch(getPeriod, search), max)" />
        </template>
    </vs-table>
</div>
</template>
...
export default {
    ...
    methods: {
        populateTable() {
            ...
            this.$store.dispatch("getPeriod", this.selectedData)
            this.activeTable = true
        },
        deleteData(id) {
            this.$store.dispatch("deletePeriod", id).then(() => {
