        ...
        <template #header>
            ...
            <div class="box" style=" margin-right : -300px; margin-top:20px">
                ...
                <select style="border:1px solid gray; border-radius : 2px solid red ; " v-model="selectedData" @change="populateTable()">
                    ...
                    <option value="" selected disabled hidden><i class='bx bx-home'></i> من فضلك اختار نوع الاشاره </option>

                    <option v-for="(i, key) in getvehiclesTypesData_3" :key="key" :value="i.VEHICLE_TYPE_ID">
                        {{ i.VEHICLE_TYPE_NAME }}
            ...
            <vs-dialog v-model="createActive">
                ...
                <div class="dialog" style="display : block ; direction : rtl ; justify-content: center ; left : -20px">
                    <div style="display : flex">
                        ...
                        <vs-input style="left : -20px ; width : 70% ;direction : rtl" v-model.lazy="name" placeholder='اسم الوقود' @keypress.enter="createData(name, typeID), createActive = false" />
                    </div>
                    <br>
...
export default {
    ...
    created() {
        this.$store.dispatch("getvehiclesTypesData_3")
    },

    computed: {
        getvehiclesTypesData_3() {
            return this.$store.getters.vehiclesTypesGetter_3
        },
        getvehiclesData() {
