...
<div style="display: flex ; justify-content:center">
    ...
    <vs-table>
        ...
        <template #tbody v-if="activeTable">
            <vs-tr :key="i" v-for="(tr, i) in $vs.getPage($vs.getSearch(unitVehiclesOil, search), page, max)" :data="tr">
                <vs-td style="width : 10% ; text-align : center ; margin-left:70px ">
                    <a @click="deleteActive = true, group_id = tr. UNIT_VEHICLE_OIL_ID">
                ...
                <vs-td style="width : 10% ; text-align : center ; margin-left:70px ">
                    <a edit @click="editActive=true, group_id = tr.VEHICLE_OIL_TYPE_NAME, group_name = tr.VEHICLE_OIL_TYPE_NAME">
                    ...
                    <vs-dialog v-model="editActive">
                        ...
                        <div class="dialog" style="display : block ; direction : rtl ; justify-content: center ; left : -20px">
                            <vs-input style="width:70% ; left : -50px" v-model.lazy="group_name" @keypress.enter="updateTable(group_id, group_name), editActive = false" />
                            <br>
                            <vs-button success style="left : -120px ; width : 25%" class="btnSave" @click="updateTable(group_id, group_name), editActive = false">حفظ</vs-button>
                        </div>
                    </vs-dialog>
                </vs-td>
                <vs-td style="width : 10% ; text-align : center ; margin-left:70px ">
...
export default {
    ...
    computed: {
        ...
        unitVehiclesOil() {
            return this.$store.getters.unitVehiclesGetter
        },
    },
    ...
    methods: {
        ...
        populateUnitTable() {
            ...
            this.$store.dispatch("unitVehiclesOil", this.selectedUnit)
            this.activeTable = true
        },

        deleteTable(group_id) {
            this.$store.dispatch("deleteUnitOilTable", group_id).then(() => {
                this.$store.dispatch("unitVehiclesOil", this.selectedUnit)
            })
        },

        updateTable(group_id, group_name) {
        ...
        createTable(group_name) {
            this.$store.dispatch("createUnitOilTable", {
